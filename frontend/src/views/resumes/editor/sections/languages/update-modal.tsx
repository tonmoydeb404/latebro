...
import { useUpdateLanguageMutation } from "@/store/features/resume/language/api";
...
const UpdateModal = (props: Props) => {
  ...
  const [mutate, response] = useUpdateLanguageMutation();
  ...
  const defaultValues = useMemo<SchemaType>(
    () => ({
      title: data?.title || "",
      experience: data?.experience || "basic",
    }),
    [data]
  );
  ...
  const onValid: SubmitHandler<SchemaType> = async (values) => {
    ...
    toast({ title: "Language record updated successfully!" });
    formOptions.reset();
    onClose();
  };
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className={"overflow-y-scroll max-h-screen"}>
        <DialogHeader className="mb-5">
          <DialogTitle>Update Language Record</DialogTitle>
        </DialogHeader>
