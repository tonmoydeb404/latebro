...
import { useCreateSocialMutation } from "@/store/features/resume/social/api";
...
const CreateModal = (props: Props) => {
  ...
  const [mutate, response] = useCreateSocialMutation();
  ...
  const onValid: SubmitHandler<SchemaType> = async (values) => {
    ...
    toast({ title: "Social record created successfully!" });
    formOptions.reset();
    setOpen(false);
  };
    ...
    <Dialog open={open} onOpenChange={setOpen} modal={true}>
      ...
      <DialogContent className={"overflow-y-scroll max-h-screen"}>
        <DialogHeader className="mb-5">
          <DialogTitle>Create Social Record</DialogTitle>
        </DialogHeader>
